...
def get_temperature_trends():
    try:
        trends = [
            {"time":trend["timestamp"],"temperature":trend["temperature"]}
            for trend in sensor_data_collection.find({},{"_id":0,"timestamp":1,"temperature":1})
        ]
...
@app.route('/api/get_thresholds', methods=['GET'])
def get_thresholds():
    try:
